...
cross_cluster_sp = shoppers_with_cluster.groupby(['cluster_number', 'SpecialDay']).size().unstack().fillna(0)
display(cross_cluster_sp)

# クラスタ番号と年齢層を軸に集計し、年齢層を列に設定
cross_cluster_r = shoppers_with_cluster.groupby(['cluster_number', 'Region']).size().unstack().fillna(0)
